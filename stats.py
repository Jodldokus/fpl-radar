...
async def init_players(understat, fpl):
    ...
    for player in players:
        ...
        new_player = Player(
            ...
            team=db.session.query(Team).filter_by(name=team_title).first(),
        )
        # 2. adds player to db
...
async def get_player_positions(fpl):
    ...
    for player in fpl_players:
        player_like_query = Player.query.filter(
            Player.name.like(f"%{player.web_name}%")
        ).all()
    ...
    for team in teams:
        ...
        for fixture in fixtures:
            ...
            is_in_db = (
                ...
                .filter_by(
                    home_team_id=get_team_id(fixture["h"]["title"]),
                    away_team_id=get_team_id(fixture["a"]["title"]),
                )
                .first()
            )
            ...
            if is_in_db:
                ...
                continue
            new_match = Match(
                home_team=db.session.query(Team)
                .filter_by(name=fixture["h"]["title"])
                .first(),
                away_team=db.session.query(Team)
                .filter_by(name=fixture["a"]["title"])
                .first(),
    ...
    for player in db.session.query(Player).all():
        ...
        for match in recent_matches[player.team_id]:
            ...
            if featured:
                ...
                new_performance = Performance(player_id=player.id, match_id=match.id)
            db.session.add(new_performance)
    return db.session.commit()
...
def player_featured(match, player_matches):
    ...
    for played_match in player_matches:
        ...
        if (
            get_team_id(played_match["h_team"]) == match.home_team_id
            and get_team_id(played_match["a_team"]) == match.away_team_id
        ):
            return played_match
    return False
